...
public class AlumnoServiceImp implements AlumnoService{
    ...
    public AlumnoDTO actualizarAlumno(AlumnoUpdateDTO alumnoUpdateDTO) {
        ...
        try {
            ...
            // Actualiza Grado
            Grado grado = gradoRepository.findById(alumnoUpdateDTO.getGradoUpdateDTO().getId())
                    .orElseThrow(() -> new EntityNotFoundException("Grado no encontrado con ID: " + alumnoUpdateDTO.getGradoUpdateDTO().getId()));
            alumnoActualizado.setGrado(grado);

            // Actualiza Cursos
            List<Cursos> cursosActualizados = new ArrayList<>();
            for (CursosUpdateDTO cursoDTO : alumnoUpdateDTO.getCursosUpdateDTO()) {
